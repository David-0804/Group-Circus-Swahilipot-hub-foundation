...
class MaintenanceLog(TimeStampedModel):
    ...
    assigned_to = models.ForeignKey('accounts.User', null=True, blank=True, on_delete=models.SET_NULL, related_name='maintenance_assignments')
...
class EquipmentItemSerializer(serializers.ModelSerializer):
    ...
    def get_current_borrower(self, obj):
        checkout = obj.checkout_requests.filter(status='active').first()
        if checkout:
...
class ReturnEquipmentView(APIView):
    def post(self, request, pk):
        try:
            checkout = CheckoutRequest.objects.get(pk=pk, item__organisation=request.user.organisation, status='active')
        except CheckoutRequest.DoesNotExist:
