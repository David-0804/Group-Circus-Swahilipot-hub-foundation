...
const DEMO_SHARED_MEDIA = [
  { id: "sm1", type: "document" as const, name: "Q2_Report.pdf", size: 412000, sharedAt: "2 days ago", url: "#" },
  { id: "sm2", type: "image" as const, name: "equipment_photo.jpg", size: 1240000, sharedAt: "3 days ago", url: "#" },
  { id: "sm3", type: "document" as const, name: "bulletin_draft.docx", size: 87000, sharedAt: "Last week", url: "#" },
  { id: "sm4", type: "image" as const, name: "studio_setup.png", size: 2100000, sharedAt: "Last week", url: "#" },
];
...
export function ContactProfile({ participant, conversationId, onClose }: Props) {
  const [activeMediaTab, setActiveMediaTab] = useState<"all" | "images" | "docs">("all");
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="flex flex-col items-center py-6 px-4 border-b border-surface-border gap-3">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-Swahilipot-400 font-medium mt-0.5">{participant.role}</p>
          </div>
          <div className={clsx(
            "badge text-xs",
        ...
        <div className="px-4 py-4">
          ...
          <div className="flex gap-1 mb-3">
            {(["all", "images", "docs"] as const).map((tab) => (
              ...
              >
                {tab === "all" ? "All Files" : tab === "images" ? "Images" : "Docs"}
              </button>
            ))}
          </div>
          ...
          <div className="space-y-2">
            {filteredMedia.length === 0 ? (
              <p className="text-xs text-slate-600 text-center py-4">No files shared yet</p>
            ) : (
...
function InfoRow({ label, value, isEmail }: { label: string; value: string; isEmail?: boolean }) {
  return (
    <div>
      ...
      {isEmail ? (
        <a href={`mailto:${value}`} className="text-xs text-Swahilipot-400 hover:underline truncate block">
          {value}
        </a>
      ) : (
