...
function CreateTaskModal({
	...
	const onSubmit = (formData: any) => {
		...
		mutation.mutate({
			...
			is_bulk: assignMode === "bulk",
			files: uploadedFiles.map(({ name, size, type }) => ({
				name,
				size,
				type,
			})),
		...
		<div className="modal-backdrop" onClick={onClose}>
			<div
				...
				<div className="modal-header">
					<h3 className="font-semibold text-white">Assign New Task</h3>
										...
										<>
											...
											{task.submission.submitted_at && (
												<p className="text-xs text-slate-500">
													Submitted: {task.submission.submitted_at}
												</p>
											)}
										</>
									) : (
